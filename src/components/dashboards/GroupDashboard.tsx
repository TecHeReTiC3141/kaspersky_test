            ...
            {groups.map(group => (
                <div key={group} className="w-72 flex-shrink-0 border border-gray-700 pt-4 pb-2 px-3">
                    ...
                    {employees.filter(employee => employee.group === group).map(employee => (
                        <GroupEntry key={employee.id} employee={employee} isSelected={selected[ employee.id ] || false}
                                    setIsSelected={setSelected}/>
                    ))}
                    <button className="text-lg text-blue-400 hover:text-blue-500 mt-4">Добавить пользователя...</button>
                </div>
            ))}
